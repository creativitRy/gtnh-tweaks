<script lang="ts">
	...
	$: filesToDownload = selectedTweakList
		.reduce((acc, t) => {
			const tweak = t as TweakDef;
		...
		<section class="panel right">
			...
			<footer>
				...
				<div class="download-notes">
					...
					<br />
					<br />1. Install GTNH v{$selectedVersion} for Java 17-25. Java 8 or CurseForge are NOT supported. (<a
					href="https://www.gtnewhorizons.com/version-history/" target="_blank">link</a>, <a
					href="https://gtnh.miraheze.org/wiki/Installing_and_Migrating" target="_blank">instructions</a>)
					<br />2. Download the generated gtnh-tweaks.zip file.
					<br />3. Unzip the zip file such that the .minecraft folder is overwritten (there should be a gtnh-changes.txt
					file next to GregTech_de_DE.lang file)
					{#if filesToDownload.length > 0}
						<br />3.5. Download the following files:
						{#each filesToDownload as file (file.filename)}
							<br />- <a href={file.url} target="_blank">{file.filename}</a> ({file.description})
						{/each}
					{/if}
					{#if filesToDelete.length > 0}
						<br />3.75. Delete the following files:
